...
export default function Stream() {
    ...
    return (
        <div className="space-y-6">
            {/* Stream Header */}
            <div
                className={`p-4 rounded-lg shadow ${stream.isArchived ? 'bg-gray-200 border-gray-300' : 'bg-white border-gray-200'}`}
            >
                <div className="flex flex-col sm:flex-row justify-between items-start gap-2">
                    <div>
                        <h1
                            className={`text-3xl font-bold ${stream.isArchived ? 'text-gray-500' : 'text-gray-800'}`}
                        >
                            {stream.name}
                        </h1>
                        <p
                            className={`text-sm ${stream.isArchived ? 'text-gray-400' : 'text-gray-500'}`}
                        >
                            Code:{' '}
                            ...
                            {stream.isArchived && (
                                <span className="ml-2 font-semibold text-gray-600">(Archived)</span>
                            )}
                        </p>
                    </div>
                    {/* Action Buttons */}
                    <div className="flex space-x-2 mt-2 sm:mt-0">
                        ...
                        {!isOwner &&
                            !stream.isArchived && ( // Members can only leave active streams
                                <Button
                                    variant="destructive"
                                    size="sm"
            ...
            <div
                className={`grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 ${stream.isArchived ? 'opacity-60 pointer-events-none' : ''}`}
            >
                {/* Link Cards */}
                <Link
                    href={`/streams/${streamId}/timetable`}
                ...
                <Link
                    href={`/streams/${streamId}/attendance`}
                    className="block bg-white p-6 rounded-lg shadow hover:shadow-md transition-shadow"
                >
                ...
                <Link
                    href={`/streams/${streamId}/analytics`}
                    className="block bg-white p-6 rounded-lg shadow hover:shadow-md transition-shadow"
                >
            ...
            <div
                className={`bg-white p-6 rounded-lg shadow ${stream.isArchived ? 'opacity-60' : ''}`}
            >
                ...
                <ul className="space-y-2 max-h-60 overflow-y-auto">
                    {stream.members.map((member) => (
                        ...
                        >
                            ...
                            <span
                                className={`text-xs font-medium px-2 py-0.5 rounded-full ${member.role === 'admin' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'}`}
                            >
                                {member.role}
                            </span>
                        </li>
                ...
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogTitle className="flex items-center">
                            <AlertTriangle className="text-red-500 mr-2" /> Confirm Leave Stream
                        </DialogTitle>{' '}
                    </DialogHeader>
                    <DialogDescription>
                        Are you sure you want to leave the stream "{stream.name}"?
                    </DialogDescription>
                    <DialogFooter>
                        <Button
                            variant="ghost"
            ...
            <Dialog open={isArchiveConfirmOpen} onOpenChange={setIsArchiveConfirmOpen}>
                <DialogContent>
                    <DialogHeader>
                        {' '}
                        <DialogTitle className="flex items-center">
                            <Archive className="text-orange-500 mr-2" /> Confirm Archive Stream
                        </DialogTitle>{' '}
                    </DialogHeader>
                    <DialogDescription>
                        Archiving "{stream.name}" will hide it from the main list for all members.
                        Timetables and attendance data will be preserved. You can unarchive it
                        later. Are you sure?
                    </DialogDescription>
                    <DialogFooter>
                        <Button
