...
const AttendanceButton: React.FC<AttendanceButtonProps> = ({
    ...
    const entryKey =
        viewEntry.recordId ||
        `${viewEntry.date}_${viewEntry.subjectName}_${viewEntry.isReplacement}`;
    ...
    return (
        ...
        >
            {isMutatingThis ? (
                ...
                <>
                    {' '}
                    <Check size={14} className="mr-1" /> Attended
                </>
            ) : (
                'Mark Attended'
            )}
...
export default function Attendance() {
    ...
    const streamStartDate = useMemo(
        () =>
            streamDetails?.streamStartDate
                ? startOfWeek(parseISO(streamDetails.streamStartDate), { weekStartsOn: 1 })
                : null,
        [streamDetails],
    );
    ...
    const currentWeekStart = useMemo(
        () => (streamStartDate ? addDays(streamStartDate, weekOffset * 7) : null),
        [streamStartDate, weekOffset],
    );
    const currentWeekEnd = useMemo(
        () => (currentWeekStart ? endOfWeek(currentWeekStart, { weekStartsOn: 1 }) : null),
        [currentWeekStart],
    );
    ...
    const groupedData = useMemo((): {
        [key: number]: { date: Date; entries: WeeklyAttendanceViewEntry[] };
    } => {
        ...
        <div className="space-y-8">
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4 border-b pb-4">
                ...
                <div className="flex items-center space-x-2">
                    <Button
                        ...
                        variant="outline"
                        size="sm"
                        disabled={isLoading || !streamStartDate || weekOffset <= 0}
                    >
                        {' '}
                    ...
                    <Button
                        ...
                        variant="outline"
                        size="sm"
                        disabled={isLoading || !streamStartDate}
                    >
                        {' '}
                        Today's Week{' '}
                    </Button>
            ...
            {currentWeekStart && currentWeekEnd && (
                <div className="text-center text-lg font-semibold text-gray-700">
                    Week {weekOffset + 1}: {format(currentWeekStart, 'MMM dd')} -{' '}
            ...
            {isLoading && (
                <div className="text-center py-10 text-gray-500 flex justify-center items-center">
                    <Loader2 className="mr-2 h-4 w-4 animate-spin" /> Loading attendance data...
                </div>
            )}
            {!isLoading && error && (
                            ...
                            >
                                ...
                                <CardContent className="p-4 space-y-3 flex-grow">
                                    ...
                                    {groupedData[dayOfWeek].entries.map((entry, index) => (
                                        <div
                                            key={`${entry.subjectName}-${entry.startTime || index}-${entry.isReplacement}`}
                                        ...
                                        >
                                            <div className="flex justify-between items-start mb-1">
                                                <div>
                                                    ...
                                                    <p
                                                        className={`text-sm font-medium ${entry.status === AttendanceStatus.CANCELLED ? 'line-through text-gray-500' : 'text-gray-900'}`}
                                                    >
                                                        {entry.subjectName}
                                                    </p>
                                                    {entry.courseCode && (
                                                ...
                                                <p
                                                    className={`text-xs font-mono whitespace-nowrap pl-1 ${entry.status === AttendanceStatus.CANCELLED ? 'text-gray-400' : 'text-gray-500'}`}
                                                >
                                                    {entry.startTime || '--:--'} -{' '}
                                                    {entry.endTime || '--:--'}
                                                </p>
            ...
            <div className="mt-8 pt-4 border-t">
                ...
                <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-gray-700">
                    <span className="inline-flex items-center">
                        <Check className="w-4 h-4 mr-1 text-green-600" /> Attended
                    </span>
                    <span className="inline-flex items-center">
                    ...
                    <span className="inline-flex items-center">
                        <X className="w-4 h-4 mr-1 text-red-600" /> Cancelled (by Admin)
                    </span>
                    <span className="inline-flex items-center">
                        <Repeat size={12} className="w-4 h-4 mr-1 text-blue-600" /> Replacement
                        Class
                    </span>
                </div>
            </div>
