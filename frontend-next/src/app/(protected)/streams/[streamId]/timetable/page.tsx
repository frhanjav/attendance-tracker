...
export default function Timetable() {
    ...
    const isAdmin = useMemo(() => {
        ...
        return (
            streamDetails.ownerId === user.id ||
            streamDetails.members.some((m) => m.userId === user.id && m.role === 'admin')
        );
    }, [user, streamDetails]);

    const { data: allTimetables = [], isLoading: isLoadingList } = useQuery<
        TimetableBasicInfo[],
        Error
    >({
    ...
    const setEndDateMutation = useMutation<
        TimetableOutput,
        Error,
        { timetableId: string; data: SetEndDateInput }
    >({
    ...
    const handleConfirmEnd = (e: React.FormEvent) => {
        ...
        setEndDateMutation.mutate({
            timetableId: timetableToEnd.id,
            data: { validUntil: endDate },
        });
    };
    ...
    if (!streamId)
        return <div className="text-center p-10 text-red-500">Invalid Stream ID provided.</div>;
            ...
            {isAdmin && (
                <Card className="shadow-md border border-gray-200 mt-8">
                    <CardHeader>
                        ...
                        <CardDescription>
                            Manage the most recently created timetable, which overrides all previous
                            ones.
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                        {isLoadingList && (
                            <p className="text-sm text-gray-500">Loading timetable details...</p>
                        )}
                        {mostRecentTimetable ? (
                            <div
                                className={`flex justify-between items-center p-3 rounded-md ${mostRecentTimetable.validUntil ? 'bg-gray-100' : 'bg-green-50 border-green-200 border'}`}
                            >
                                <div>
                                    ...
                                    <p className="text-xs text-gray-600">
                                        Active From:{' '}
                                        {format(
                                            parseISO(mostRecentTimetable.validFrom),
                                            'MMM dd, yyyy',
                                        )}{' '}
                                        | Ends:{' '}
                                        {mostRecentTimetable.validUntil
                                            ? format(
                                                  parseISO(mostRecentTimetable.validUntil),
                                                  'MMM dd, yyyy',
                                              )
                                            : 'Present'}
                                    </p>
                                ...
                                {!mostRecentTimetable.validUntil && (
                                    <Button
                                        variant="outline"
                                        size="sm"
                                        onClick={() => openEndModal(mostRecentTimetable)}
                                    >
                            ...
                            !isLoadingList && (
                                <p className="text-sm text-gray-500 italic p-3">
                                    No timetables have been created for this stream yet.
                                </p>
                            )
                        )}
                    </CardContent>
            ...
            <Dialog open={isEndModalOpen} onOpenChange={(open) => !open && handleCloseEndModal()}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            End the timetable "{timetableToEnd?.name}". No more classes will be
                            scheduled by this timetable after the selected date.
                        </DialogDescription>
                    </DialogHeader>
                    <form onSubmit={handleConfirmEnd} className="py-4 space-y-4">
                        <div>
                            ...
                            <Input
                                ...
                                min={
                                    timetableToEnd
                                        ? format(parseISO(timetableToEnd.validFrom), 'yyyy-MM-dd')
                                        : undefined
                                }
                                required
