...
const NavLink: React.FC<{
    ...
    className?: (params: { isActive: boolean }) => string;
}> = ({ href, children, className }) => {
    const pathname = usePathname();
    const isActive = pathname === href;

    return (
        <Link href={href} className={className ? className({ isActive }) : undefined}>
            {children}
        </Link>
    );
};
...
const Sidebar: React.FC = () => {
    ...
    const activeStreamId = params?.streamId as string;
                            ...
                            <AccordionItem value={stream.id} key={stream.id} className="border-b-0">
                                ...
                                <AccordionContent className="pt-1 pb-2 pl-4 pr-1">
                                    <nav className="flex flex-col space-y-1">
                                        <NavLink
                                            href={`/streams/${stream.id}/timetable`}
                                            className={({ isActive }) =>
                                        ...
                                        <NavLink
                                            href={`/streams/${stream.id}/attendance`}
                                            className={({ isActive }) =>
                                                `flex items-center px-2 py-1 rounded text-xs ${isActive ? 'bg-blue-50 text-blue-700 font-medium' : 'text-gray-500 hover:bg-gray-100 hover:text-gray-800'}`
                                            }
                                        >
                                        ...
                                        <NavLink
                                            href={`/streams/${stream.id}/analytics`}
                                            className={({ isActive }) =>
                                                `flex items-center px-2 py-1 rounded text-xs ${isActive ? 'bg-blue-50 text-blue-700 font-medium' : 'text-gray-500 hover:bg-gray-100 hover:text-gray-800'}`
                                            }
                                        >
