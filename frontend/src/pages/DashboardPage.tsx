        ...
        <div>
            ...
            <div className="bg-white p-6 rounded-lg shadow">
                ...
                {!isLoading && !error && streams && streams.length > 0 ? (
                    <ul className="space-y-3">
                        {streams.map(stream => (
                            <li key={stream.id} className="border p-3 rounded hover:bg-gray-50">
                                <Link to={`/streams/${stream.id}`} className="text-blue-700 hover:underline font-medium block">
                                    {stream.name}
                                </Link>
                                <span className="text-xs text-gray-500 block mt-1">Code: {stream.streamCode}</span>
                 ...
                 {!isLoading && !error && (!streams || streams.length === 0) && (
                    <p className="text-gray-500 text-center py-4">You haven't joined or created any streams yet. Use the buttons above to start!</p>
                )}
            </div>

            {/* Create Stream Modal */}
            <Modal isOpen={isCreateModalOpen} onClose={() => setCreateModalOpen(false)} title="Create New Stream">
                 <form onSubmit={handleCreateSubmit} className="space-y-4">
                     <div>
                        <label htmlFor="streamName" className="block text-sm font-medium text-gray-700 mb-1">Stream Name</label>
                        <input
                            id="streamName"
                            type="text"
             ...
             {/* Join Stream Modal */}
            <Modal isOpen={isJoinModalOpen} onClose={() => setJoinModalOpen(false)} title="Join Existing Stream">
