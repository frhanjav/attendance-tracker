...
const Sidebar: React.FC<SidebarProps> = ({ openCreateStreamModal, openJoinStreamModal }) => {
    ...
    const { data: streams, isLoading, error } = useQuery({
        queryKey: ['myStreams'], // Use the same key as Dashboard if needed
        queryFn: streamService.getMyStreams,
        staleTime: 1000 * 60 * 10, // Cache streams for 10 mins
    });
    ...
    return (
        <aside className="w-64 bg-white border-r border-gray-200 flex flex-col flex-shrink-0 h-[calc(100vh-theme(space.16))]"> {/* Adjust height based on Navbar */}
            {/* Top Links */}
                            ...
                            <AccordionItem value={stream.id} key={stream.id} className="border-b-0">
                                <AccordionTrigger
                                    className={`px-2 py-1.5 text-sm hover:bg-gray-100 rounded-md ${
                                        activeStreamId === stream.id ? 'font-semibold text-blue-700' : 'text-gray-700'
                                    }`}
                                >
                                    <Link to={`/streams/${stream.id}`} className="flex-1 text-left truncate" title={stream.name}>
                                        {stream.name}
                                    </Link>
                                </AccordionTrigger>
                                <AccordionContent className="pt-1 pb-2 pl-4 pr-1">
                                    <nav className="flex flex-col space-y-1">
                                        <NavLink
                                            ...
                                            className={({ isActive }) => `flex items-center px-2 py-1 rounded text-xs ${isActive ? 'bg-blue-50 text-blue-700 font-medium' : 'text-gray-500 hover:bg-gray-100 hover:text-gray-800'}`}
                                        >
                                        ...
                                        <NavLink
                                            ...
                                            className={({ isActive }) => `flex items-center px-2 py-1 rounded text-xs ${isActive ? 'bg-blue-50 text-blue-700 font-medium' : 'text-gray-500 hover:bg-gray-100 hover:text-gray-800'}`}
                                        >
                                            <BarChartHorizontal className="mr-2 h-3.5 w-3.5" /> Analytics
                                        </NavLink>
                                    </nav>
                                </AccordionContent>
                            </AccordionItem>
            ...
            <div className="p-4 border-t mt-auto space-y-2">
                 <Button size="sm" variant="outline" className="w-full justify-start text-xs" onClick={openCreateStreamModal}> {/* Add onClick */}
                     <PlusCircle size={14} className="mr-2"/> Create Stream
                 </Button>
                 <Button size="sm" variant="outline" className="w-full justify-start text-xs" onClick={openJoinStreamModal}> {/* Add onClick */}
